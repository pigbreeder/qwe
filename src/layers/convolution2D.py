...
class Convolution2D(Layer):
    ...
    def forward(self, X):
        return speed_forward(X,self.W,self.b,self.output_size,self.stride,self.pad)
    ...
    def backward(self, X, dA):
        pass
        return speed_backward(X,self.W,self.b,dA,self.pad)
...
@nb.jit()
def speed_backward(X, W, b,dA, pad):
    m, n_H, n_W, n_C = dA.shape
